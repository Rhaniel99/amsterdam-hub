...
async function handleSelection(interaction) {
  ...
  const shortcuts = await Shortcuts.findAll({ where: { ToolId: selectedValue } });
  const embedCommand = new EmbedBuilder()
  .setColor('#0099ff')
  .setTitle(`Lista de Commandos - ${tool.name}`);
  embedCommand.addFields({ name: '\u200B', value: '\u200B' });
  shortcuts.forEach(command => {
    embedCommand.addFields(
    { name: command.name, value: command.command, inline: false } 
  );
});
...
module.exports = {
  ...
  async execute(interaction) {
    const toolsList = await Tools.findAll({ attributes: ['name', 'id'] });
    ...
    toolsList.forEach(tool => {
      selectOptions.push(
        new StringSelectMenuOptionBuilder()
          ...
          .setDescription('Selecione esta ferramenta') // Descrição opcional
    ...
    const select = new StringSelectMenuBuilder()
      .setCustomId('starter')
      .setPlaceholder('Selecione uma ferramenta')
